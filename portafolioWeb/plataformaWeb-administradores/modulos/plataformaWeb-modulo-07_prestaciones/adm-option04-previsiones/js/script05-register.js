...
function loadDoctorsRealTime() {
    showLoadingSpinner(); 
    onSnapshot(collection(db, 'previsiones'), (snapshot) => {
        let docs = [];
...
btnSave.addEventListener('click', async (e) => {
    ...
    if (!doctorName) {
        showMessage(messageWarning, warningText, 'Advertencia: Ingrese el nombre de la previsión');
        return;
    }
    ...
    try {
        ...
        hideSpinner();
        showMessage(messageSuccess, successText, `La previsión ${doctorName} se ha registrado con éxito`);
        formRegister.reset();
...
async function deleteDoctor(docId, nombre, row) {
    try {
        ...
        hideDeleteSpinner();
        showMessage(messageSuccess, successText, `La previsión ${nombre} se ha eliminado con éxito`);
