...
btnImportar.addEventListener('click', async () => {
    ...
    try {
        ...
        const medicoIndex = headers.findIndex(header => String(header).toLowerCase() === 'medico');
        if (medicoIndex === -1) {
            alert("La columna 'Medico' no se encontró en el archivo.");
            hideImportSpinner();
            return;
        }
        ...
        hideImportSpinner();
        showMessage(messageSuccess, successText, `Se han importado ${importedCount} filas con nuevos médicos con éxito.`);
        importContainer.classList.add('hidden');
        fileInput.value = "";
    } catch (error) {
