...
btnImportar.addEventListener('click', async () => {
    ...
    try {
        ...
        const empresaIndex = headers.findIndex(header => String(header).toLowerCase() === 'empresa');
        const rutIndex = headers.findIndex(header => String(header).toLowerCase() === 'rut');
        
        if (empresaIndex === -1 || rutIndex === -1) {
            alert("Las columnas 'empresa' y/o 'rut' no se encontraron en el archivo.");
            hideImportSpinner();
            return;
        }
        ...
        for (let i = 1; i < jsonData.length; i++) {
            ...
            const empresaName = row[empresaIndex];
            const rut = row[rutIndex];
            
            if (!empresaName || !rut) continue; 
        ...
        hideImportSpinner();
        showMessage(messageSuccess, successText, `Se han importado ${importedCount} filas con nuevas empresas con éxito.`);
        importContainer.classList.add('hidden');
        fileInput.value = "";
    } catch (error) {
