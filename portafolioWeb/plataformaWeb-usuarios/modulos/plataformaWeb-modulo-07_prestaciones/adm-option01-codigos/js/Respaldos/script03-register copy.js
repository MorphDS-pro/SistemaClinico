...
function renderPage() {
    if (currentTab === 'codigos') {
        const totalRecords = allCodigos.length;
        const totalPages = Math.ceil(totalRecords / pageSize) || 1;
        if (currentPage > totalPages) currentPage = totalPages;
        const start = (currentPage - 1) * pageSize;
        const pageRecords = allCodigos.slice(start, start + pageSize);
        tableBody.innerHTML = '';
        ...
        const totalRecords = allPendientes.length;
        const totalPages = Math.ceil(totalRecords / pageSize) || 1;
        if (currentPage > totalPages) currentPage = totalPages;
        const start = (currentPage - 1) * pageSize;
        const pageRecords = allPendientes.slice(start, start + pageSize);
        pendientesTableBody.innerHTML = '';
        pageRecords.forEach(item => {
            const { docId, data } = item;
            const row = document.createElement('tr');
...
// Función de eventos para los botones de paginación
